...
**Goal:** 在项目根目录创建一份准确、易读的中文 `README.md`，用于 GitHub 项目展示和新用户快速上手。

**Architecture:** README 采用“项目价值 → 功能能力 → 快速开始 → 实际使用 → 模型约束 → 开发维护”的阅读路径。所有事实直接对应当前 Android 配置、模型实现、局域网 Web 服务和根目录脚本，不增加新的运行时功能。

**Tech Stack:** Markdown、Android、Kotlin、Jetpack Compose、CameraX、TensorFlow Lite Task Vision、Room、NanoHTTPD、Gradle Wrapper 9.3.1。

## Global Constraints

- README 以中文为主，命令、文件名和技术名词保留原文。
- 当前唯一内置模型必须写为 SSD MobileNet V1，文件名为 `mobilenet_ssd.tflite`。
- PC 控制台默认地址必须写为 `http://手机IP:8080`。
- 自训练 YOLO 仅描述为导入口，并明确要求兼容 TensorFlow Lite Task Vision `ObjectDetector` 和 Metadata。
- 最低 Android 版本必须写为 Android 7.0（API 24）。
- 普通构建使用 JDK 17；Android SDK 36 的 Robolectric 测试使用 Java 21 或更高版本。
- 不添加截图占位、APK 下载地址、许可证声明、精度指标或未实现能力。
...
**Files:**
- Create: `README.md`
- Reference: `docs/superpowers/specs/2026-07-23-readme-design.md`
- Test: PowerShell 内容验证命令

**Interfaces:**
- Consumes: 当前项目的 Gradle Wrapper、`打包.bat`、`安装.bat`、`mobilenet_ssd.tflite`、手机端页面和 8080 端口 Web 服务。
- Produces: GitHub 和本地开发者可直接阅读的根目录 `README.md`。
...
if (Test-Path -LiteralPath .\README.md) {
    throw "README.md should not exist before implementation"
}
Write-Error "README.md is missing"
```

Expected: FAIL with `README.md is missing`，证明验证针对待创建文件。

- [ ] **Step 2: 创建 README**

Create `README.md` with the following exact content:
...
foreach ($value in $required) {
    if (-not $readme.Contains($value)) {
        throw "README missing required content: $value"
    }
}

$paths = @(
    ".\app\src\main\assets\mobilenet_ssd.tflite",
    ".\gradle\wrapper\gradle-wrapper.properties",
    ".\tests\BatchScripts.Tests.ps1",
    ".\打包.bat",
    ".\安装.bat"
)
...
foreach ($path in $paths) {
    if (-not (Test-Path -LiteralPath $path)) {
        throw "README references a missing file: $path"
    }
}

$unfinishedMarkers = @("TO" + "DO", "TB" + "D")
foreach ($marker in $unfinishedMarkers) {
    if ($readme -match "\b$marker\b") {
        throw "README contains an unfinished placeholder: $marker"
    }
}

Write-Output "README validation passed"
```
...
Expected: exit code `0`，没有空白错误。

- [ ] **Step 5: 提交 README**

```powershell
git add README.md docs/superpowers/plans/2026-07-23-readme.md
git commit -m "docs: add project README"
```

Expected: 提交包含根目录 `README.md` 和本实施计划。
